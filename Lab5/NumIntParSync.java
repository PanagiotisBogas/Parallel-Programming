public class NumIntParSync {
    public static void main(String[] args) throws InterruptedException {
        ...
        NumIntParSyncThread[] threads = new NumIntParSyncThread[numThreads];

        for (int i = 0; i < numThreads; i++) {
            threads[i] = new NumIntParSyncThread(i, numSteps, numThreads, step, sum);
            threads[i].start();
        }
...
class NumIntParSyncThread extends Thread{
    sumObj globalSum;
    int tId;
    double tStep;
    int block;
    ...
    public NumIntParSyncThread(int i, long numSteps, int numThreads, double step, sumObj sum){
        globalSum = sum;
        tId = i;
        tStep=step;
